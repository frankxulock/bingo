...
module.exports = {
  ...
  settings: {
    ...
    gitCommitMessage: '',        // git提交信息（空则使用默认）
    gitForceMode: 'auto'         // git冲突处理模式: 'auto'(自动), 'force'(强制), 'pull'(拉取)
  },
  ...
  messages: {
    ...
    async 'editor:build-finished'(event, buildOptions) {
      ...
      try {
        ...
        // 2. 拷贝 url-decrypt.js 文件（仅在使用 preview-templates 时）
        ...
        if (this.settings.gitAutoUpdate) {
          ...
          // 构建输出目录通常是 build/dev/web-mobile，但Git仓库在 build/dev
          // 所以我们需要使用 outputDir 的父目录作为Git仓库路径
          const gitRepoPath = path.dirname(outputDir);
          
          const gitOptions = {
            projectPath: gitRepoPath,    // Git仓库路径 (build/dev)
            buildPath: outputDir,        // 构建输出路径 (build/dev/web-mobile)
          ...
          logger.log(`📁 Git 仓库路径: ${gitRepoPath}`);
          logger.log(`📦 构建输出路径: ${outputDir}`);
          logger.log(`🌿 目标分支: ${gitOptions.targetBranch}`);
      ...
      if (typeof Editor !== 'undefined') {
        try {
          if (Editor.Ipc && typeof Editor.Ipc.sendToPanel === 'function') {
            Editor.Ipc.sendToPanel('auto-publish.settings', 'auto-publish:settings-loaded', settings);
            logger.log('✅ 设置已通过 Editor.Ipc.sendToPanel 发送');
          } else if (typeof Editor.sendToPanel === 'function') {
            Editor.sendToPanel('auto-publish.settings', 'auto-publish:settings-loaded', settings);
    ...
    'auto-publish:save-settings'(event, newSettings) {
      ...
      if (typeof Editor !== 'undefined') {
        try {
          if (Editor.Ipc && typeof Editor.Ipc.sendToPanel === 'function') {
            Editor.Ipc.sendToPanel('auto-publish.settings', 'auto-publish:settings-saved', success);
            logger.log('✅ 保存结果已通过 Editor.Ipc.sendToPanel 发送');
          } else if (typeof Editor.sendToPanel === 'function') {
            Editor.sendToPanel('auto-publish.settings', 'auto-publish:settings-saved', success);
